...
def save_case(report: Dict[str, Any]) -> str:
    """
    ...
    INSERT INTO cases (
        ...
        status, officer_notes, report_json
    ) VALUES (
        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING_REVIEW', '', ?
    )
    ...
    """, (
        case_id, filename, language, transcript, svi_score, risk_category,
        predicted_class, primary_action, urgency_level, dominant_emotion,
        emotion_confidence, acoustic_panic, linguistic_threat, report_json_str
    ))

    conn.commit()
    conn.close()
    logger.info(f"Case {case_id} saved to database.")
    return case_id
